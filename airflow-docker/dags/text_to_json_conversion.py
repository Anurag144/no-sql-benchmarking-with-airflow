...
def text_to_json_Profiles_table():
    ...
    with open('./dags/soc-pokec-profiles.txt', 'r', encoding="utf8") as data:
        for line in data:
            line = line.strip('\n')
            ldata = line.split('\t')
            if ldata[0] == "500":
    ...
    with open('./dags/data.json', 'w') as fp:
        json.dump(profile_data, fp, indent=4)
    #from pprint import pprint
    #pprint(profile_data)

    os.remove("./dags/soc-pokec-profiles.txt")
...
def text_to_json_Relationship_table():
    ...
    with open('./dags/soc-pokec-relationships.txt', 'r', encoding="utf8") as data:
        for line in data:
            line = line.strip('\n')
            ldata = line.split('\t')
    ...
    with open('./dags/relations.json', 'w') as fp:
        json.dump(relationship_data, fp, indent=4)
    #from pprint import pprint
    #pprint(relationship_data)

    os.remove("./dags/soc-pokec-relationships.txt")
